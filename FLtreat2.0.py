...
class Ui(QtWidgets.QMainWindow, Form):
    ...
    def setup_data(self):
        self.paths = functions.set_paths('firecon')
        self.rnd, self.mesh, self.min_height, self.max_height = functions.read_rnd_new(self.paths['geometry'])
        self.geom_files = list(self.mesh.keys())
        self.data_fields = functions.read_data(self.paths) 
        self.combust = functions.find_combust(self.data_fields)
        self.treat_combust = functions.treat_combust(self.combust)
    ...
    def update_table(self):
        ...
        if len(stv.sensor_points) > 0:
            ...
            x = (stv.sensor_points[-1][0] / dx * 0.495) - 22.5
            y = (stv.sensor_points[-1][1] / dy * 0.495) - 22.5
            z = stv.sensor_points[-1][2]

            element_number = len(stv.sensor_points) - 1

            self.table.setItem(element_number, 0, QtWidgets.QTableWidgetItem('%1.2f' % x))
            self.table.setItem(element_number, 1, QtWidgets.QTableWidgetItem('%1.2f' % y))
            self.table.setItem(element_number, 2, QtWidgets.QTableWidgetItem('%1.2f' % z))
    ...
    def change_minO2_slider(self):
        ...
        self.treat_combust = functions.treat_combust(self.combust, O2_min_concentration / 100)
    ...
    def button_writeClicked(self):
        ...
        self.paths = functions.set_paths(self.task_name)
    ...
    def write_rnd(self):
        ...
        for file in rnd_file:
            with open(file, 'w') as f:
                ...
                x = (stv.init_combust_point_coord[0] / dx * 0.495) - 22.5
                y = (stv.init_combust_point_coord[1] / dy * 0.495) - 22.5
                z = stv.init_combust_point_coord[2]
    ...
    def write_sostav(self):
        ...
        pd.DataFrame(sostav).set_index('N').to_csv(sostav_file, sep=' ')
